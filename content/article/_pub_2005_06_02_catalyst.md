...
Catalyst has many prebuilt components and plugins for common modules and tasks. For example, there are `View` classes available for Template Toolkit, HTML::Template, Mason, Petal, and PSP. Plugins are available for dozens of applications and functions, including [Data::FormValidator](http://search.cpan.org/perldoc?Data::FormValidator), authentication based on LDAP or [Class::DBI](http://search.cpan.org/perldoc?Class::DBI), several caching modules, [HTML::FillInForm](http://search.cpan.org/perldoc?HTML::FillInForm), and XML-RPC.
