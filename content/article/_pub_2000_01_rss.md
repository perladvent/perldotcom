...
At MNN, the user can choose what content to put on their own page—the latest headlines on [Slashdot](http://slashdot.org/), recently uploaded files at [freshmeat](http://www.freshmeat.net/), or the most recent posts to a bulletin board at [Network54](http://network54.com/). Each *channel*, as Netscape calls them, can also include information to display an image for that channel and a text input box for searching the channel's site.
...
Jonathan Eisenzopf, who has worked on a lot of XML projects for Perl and who runs [the `perlxml.com` web site](http://perlxml.com/), wrote [the `XML::RSS` module](http://search.cpan.org/search?module=XML::RSS). It is based on `XML::Parser`, as most XML modules are, and uses an object-oriented syntax. It makes creation and parsing of RSS files easy.
