...
### [Subroutines and Other Code Objects]{#subroutines_and_other_code_objects}
...
#### [Standard Subroutines]{#standard_subroutines}
...
"Trait" is the new name for a compile-time (`is`) property. See [Traits
and Properties](#traits_and_properties)

#### [Perl5ish Subroutine Declarations]{#perl5ish_subroutine_declarations}
...
If you need to modify the elements of `@_`, then declare it with the
[`is rw`](#item_is_rw) trait:
...
The arrow operator `->` is almost a synonym for the anonymous `sub`
keyword. The parameter list of a pointy sub does not require parentheses
and a pointy sub may not be given traits.
...
#### [Globally Scoped Subroutines]{#globally_scoped_subroutines}
...
Subroutines are specified as being lvalue using the
[`is rw`](#item_is_rw) trait.
...
If the left and right delimiters aren't separated by "`...`", then the
`DELIMITERS` string must have an even number of characters. The first
half is treated as the opening delimiter and the second half as the
closing.
...
### [Parameters and Arguments]{#parameters_and_arguments}
...
modification, use the [`is rw`](#item_is_rw) trait. To pass-by-copy, use
the [`is copy`](#item_is_copy) trait.

Parameters may be required or optional. They may be passed by position,
or by name. Individual parameters may confer a scalar or list context on
...
#### [Invocant Parameters]{#invocant_parameters}
...
Multimethod invocants are specified at the start of the parameter list,
with a colon terminating the list of invocants:
...
colon after them. The colon is just a special form of the comma, and has
the same precedence:
...
The first invocant is always the topic of the corresponding method or
multimethod.
...
Default values can be calculated at run-time. They can even use the
values of preceding parameters:

        sub xml_tag ($tag, ?$endtag = matching_tag($tag) ) {...}

Arguments that correspond to optional parameters are evaluated in scalar
...
Named variadic arguments are bound to the variadic hash (`*%flag` in the
above example). Such arguments are evaluated in scalar context. Any
remaining variadic arguments at the end of the argument list are bound
to the variadic array (`*@data` above) and are evaluated in list
context.
...
#### [Flattening Argument Lists]{#flattening_argument_lists}
...
The variadic array of a subroutine call can be passed in separately from
the normal argument list, by using either of the "pipe" operators: `<==`
or `==>`.
...
#### [Closure Parameters]{#closure_parameters}
...
#### [Unpacking Array Parameters]{#unpacking_array_parameters}
...
#### [Placeholder Variables]{#placeholder_variables}
...
The variable type specifies how the variable itself is implemented. It
...
#### [Hierarchical Types]{#hierarchical_types}
...
If a subroutine is not explicitly scoped, then it belongs to the current
...
### [Properties and Traits]{#properties_and_traits}

Compile-time properties are now called "traits." The `is NAME (DATA)`
...
These traits may be declared on the subroutine as a whole (not on
individual parameters).

**[`is signature`]{#item_is_signature}**\
:   The signature of a subroutine -- normally declared implicitly, by
    providing a parameter list and/or return type.

**[`returns`/`is returns`]{#item_returns%2fis_returns}**\
:   The type returned by a subroutine.

**[`will do`]{#item_will_do}**\
...
**[`is parsed`]{#item_is_parsed}**\
:   Specifies the rule by which a macro call is parsed.

**[`is cached`]{#item_is_cached}**\
:   Marks a subroutine as being memoized

**[`is inline`]{#item_is_inline}**\
:   *Suggests* to the compiler that the subroutine is a candidate for
    optimization via inlining.

**[`is tighter`/`is looser`/`is equiv`]{#item_is_tighter%2fis_looser%2fis_equiv}**\
:   Specifies the precedence of an operator relative to an existing
    operator.

**[`is assoc`]{#item_is_assoc}**\
:   Specifies the associativity of an operator.

**[`PRE`/`POST`]{#item_pre%2fpost}**\
:   Mark blocks that are to be unconditionally executed before/after the
    subroutine's `do` block. These blocks must return a true value,
    otherwise an exception is thrown.

**[`FIRST`/`LAST`/`NEXT`/`KEEP`/`UNDO`/etc.]{#item_first%2flast%2fnext%2fkeep%2fundo%2fetc%2e}**\
:   Mark blocks that are to be conditionally executed before or after
    the subroutine's `do` block. The return values of these blocks are
    ignored.
...
:   Specifies that the parameter is passed by reference. Unlike
    [`is rw`](#item_is_rw), the corresponding argument must already be a
    suitable lvalue. No attempt at coercion or autovivification is made.

**[`is copy`]{#item_is_copy}**\
:   Specifies that the parameter receives a distinct, read-writeable
    copy of the original argument. This is commonly known as
    "pass-by-value."
:       sub reprint ($text, $count is copy) {
                print $text while $count-->0;
            }

**[`is context(TYPE)`]{#item_context}**\
...
### [Advanced Subroutine Features]{#advanced_subroutine_features}

#### [The `&_` routine]{#the_&__routine}
...
#### [The `leave` Function]{#the_leave_function}

A `return` statement causes the innermost surrounding subroutine,
method, rule, macro or multimethod to return.

To return from other types of code structures, the `leave` funtion is
used:
...
The `temp` function temporarily replaces a variable, subroutine or other
...
value of the object. At the end of the lexical scope in which the `temp`
was applied, the subroutine returned by the `.TEMP` method is executed.
...
original (unwrapped) version of the subroutine. In other words, the call
to `.wrap` has more or less the same effect as:
...
#### [Anonymous Hashes vs. Blocks]{#anonymous_hashes_vs_blocks}

`{...}` is always a block/closure unless it consists of a single list,
the first element of which is either a hash or a pair.
...
The standard `hash` function takes a block, evaluates it in list
context, and constructs an anonymous hash from the resulting key/value
list:
...
#### [Pairs as lvalues]{#pairs_as_lvalues}
...
#### [Out-of-Scope Names]{#outofscope_names}

`$CALLER::varname` specifies the `$varname` visible in the dynamic scope
from which the current block/closure/subroutine was called.
