{
   ...
   "draft" : null,
   "slug" : "/pub/2003/12/p6pdigest/20031207",
   "tags" : [
...
#### [Parrot build system tinkering]{#Parrot_build_system_tinkering}
...
`Configure.pl --maintainer`{lang="und" lang="und"} is run, on the
grounds that a maintainer shouldn't be building from a read only source.
Dan asked for a volunteer to fix things up.

[http://groups.google.com/](http://groups.google.com/groups?selm=Pine.SOL.4.53.0312010940540.18708@maxwell.phys.lafayette.edu)

#### [Object stuff]{#Object_stuff}
...
[http://groups.google.com/](http://groups.google.com/groups?selm=a06010202bbf0fd3af8cc@%5B10.0.1.2%5D)
-- Rumblings

[http://groups.google.com/](http://groups.google.com/groups?selm=a06010216bbf259ac617f@%5B10.0.1.2%5D)
-- Announcement

[http://groups.google.com/](http://groups.google.com/groups?selm=a06010217bbf261e24e2a@%5B10.0.1.2%5D)
-- The way forward

[http://groups.google.com/](http://groups.google.com/groups?selm=a06010202bbf81f06483c@%5B10.0.1.2%5D)
-- Dan gets mugged by flu

[http://groups.google.com/](http://groups.google.com/groups?selm=3FD0AEE4.3000500@toetsch.at)
-- Leo's patches

#### [Raising exceptions]{#Raising_exceptions}

Cory Spencer wondered about 'proper exception raising' and wondered what
the state of play was, and if there were any good examples. Leo TÃ¶tsch
...
#### [Compilation units and the boundaries of IMCC]{#Compilation_units_and_the_boundaries_of_IMCC}
...
`printf`{lang="und" lang="und"}/`sprintf`{lang="und" lang="und"}
equivalent, implemented in C for speed. Dan seemed to agree and reckoned
it should be addressed as soon as objects were out of the way.

[http://groups.google.com/](http://groups.google.com/groups?selm=5.1.1.6.2.20031201221202.0486c298@pop.mindspring.com)

#### [IMCC pending changes]{#IMCC_pending_changes}
...
#### [PMC Compiler 2nd edition]{#PMC_Compiler_2nd_edition}
...
*pmc2c2* was definitely stable enough, but wasn't too sure about
*ops2c2*. Jonathan Worthington pointed out a Win32 bug that was quickly
fixed.

[http://groups.google.com/](http://groups.google.com/groups?selm=3FCDB6A3.4010901@toetsch.at)

#### [`get_pmc_keyed()`{lang="und" lang="und"} in PerlString]{#get_pmc_keyed()_in_PerlString}
...
`get_pmc_keyed()`{lang="und" lang="und"} on the PerlString vtable to
allow for similar tricks with the standard string PMC. I'm confused as
to what the decision was. I think Leo agreed that it was a good idea,
but that it should probably be implemented in a new PMC for the time
being...

[http://groups.google.com/](http://groups.google.com/groups?selm=20031202200415.GA6856@bumblebury.com)

#### [Symbolic vs. Named variable register allocation]{#Symbolic_vs._Named_variable_register_allocation}
...
#### [Testing for null]{#Testing_for_null}

JÃ¼rgen BÃ¶mmels is working on having the Parrot IO system's
`open`{lang="und" lang="und"} return a PMCNULL on failure instead of an
invalid IO-Object (sounds like a good idea to me). The catch is, there
doesn't seem to be any way for the bytecode to detect that this has
happened. There was some debate about whether what JÃ¼rgen was doing was
the right thing, or if we shouldn't introduce a new null.pmc, or some
...
hadn't been finally decided upon (but `if_null`{lang="und" lang="und"}
and `unless_null`{lang="und" lang="und"} are the front runners).
...
so he posted a patch to add an `eq_addr`{lang="und" lang="und"} opcode.
Melvin Smith still wasn't sure that the name was quite right though and
asked for a description of why Cory needed it (he needs it for
...
#### [Fixing the Makefile dependencies]{#Fixing_the_Makefile_dependencies}
...
for Perl 5.005\_04 and will not be appearing as a running joke in these
...
also welcome feedback at or at my website.
