...
Nathan Gray wondered if he should change his tests log to YAML output. Stevan Little pointed him to [Test::Harness::Straps](http://search.cpan.org/perldoc?Test::Harness::Straps), which can collect test output and transform it.
...
Chip asked if there was a reason that the `TOUCH` variable doesn't use `utime`. Michael G. Schwern suggested [ExtUtils::Command](http://search.cpan.org/perldoc?ExtUtils::Command). Steve Peters points out that `utime` works only on existing files.
