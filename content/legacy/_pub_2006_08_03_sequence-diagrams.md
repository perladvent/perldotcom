...
[`UML::Sequence::PerlSeq`](https://metacpan.org/pod/UML::Sequence::PerlSeq) uses the Perl debugger's hooks to profile the code as it runs, watching for the methods listed in *roller.methods*. The result is an XML file describing the calls that actually happened during this run.

To turn this into a picture, use one of the image scripts:

    $ seq2svg.pl roller.xml > roller.svg

Obviously, `seq2svg.pl` makes SVG images. If you have no way to view those, get Firefox 1.5, use a tool like the batik rasterizer, or use `seq2rast.pl`, which makes PNG images directly using the [`GD`](https://metacpan.org/pod/GD) module.

If you want diagrams like Figure 2, use [`UML::Sequence::PerlOOSeq`](https://metacpan.org/pod/UML::Sequence::PerlOOSeq) in place of `UML::Sequence::PerlSeq` when you run `genericseq.pl`.
...
Then run `genericseq.pl` with [UML::Sequence::JavaSeq](https://metacpan.org/pod/UML::Sequence::JavaSeq) in place of `UML::Sequence::PerlSeq`. Of course, this requires you to have a Java development environment on your machine. In particular, it must be able to find *tools.jar*, which provides the debugger hooks necessary to watch the calls.
