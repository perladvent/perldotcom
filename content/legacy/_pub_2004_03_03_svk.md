...
Fortunately the Subversion developers made the code easy and ready for wrapping into different languages using SWIG. At that time, only Python bindings were implemented, so I had to do the Perl bindings myself. With the Perl bindings implemented rapidly, VCP gets much faster, and I also started writing [`SVN::Mirror`](https://metacpan.org/pod/SVN::Mirror/), a module that enables mirroring between Subversion repositories. When I felt bored, I would add Subversion back-end support to tools like Bloxsom and Kwiki.
...
Additionally, with Perl's flexible PerlIO layers system, keyword expansion (like `$Id$` in `cvs`) was done within one hour. The reusable part of this was abstracted out to the [`PerlIO::via::dynamic`](https://metacpan.org/pod/PerlIO::via::dynamic) module on CPAN.
