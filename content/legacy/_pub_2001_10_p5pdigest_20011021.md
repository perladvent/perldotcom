...
Jarkko posted a [list of things to do](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00391.html) before the 5.8.0 release (later, he posted another, [shorter TODO list](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00971.html).) The main remaining issues are:
...
Nick Ing-Simmons, who was unable to work on Perl for a while because of intellectual property restrictions at his new job, is back, and working very hard on [fixing the PerlIO issues](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00887.html). Arthur Bergman has quietly but steadily been making `threads::shared` work well enough to be shipped with 5.8.0. Damian has suitably evil plans for attributes, which Arthur and I plan to assimilate into the core.
...
When Perl is running in taint mode (because it was started with the `-T` switch), a global variable (in the core) named `PL_tainting` is set to 1 (it is usually 0). Michael G. Schwern posted [a patch to make this value accessible to Perl programs](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00316.html) as `${^TAINT}`. Jarkko said:

> &lt;troll&gt;Why ${^TAINT} is read-only? It would be \*so\* convenient to ...&lt;/troll&gt;

Of course, my HTML-hating email program stripped the &lt;troll&gt; tags, so I had to post [a patch to make ${^TAINT} writable](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00347.html) (thereby making `local ${^TAINT} = 0` work as expected). The patch made Jarkko go temporarily blind (but not blind enough to accidentally apply it).
...
Paul Johnson posted a message about [some weird AUTOLOAD behaviour](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00811.html). What should the following code do?
...
Michael G. Schwern released [Test::Simple 0.32](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00612.html). In the process, he introduced `$ENV{PERL_CORE}`, to enable modules distributed both in the core and on the CPAN to use the same tests everywhere, thereby simplifying life greatly.

Johan Vromans released [Getopt::Long 2.26\_02](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00846.html), a prerelease of version 2.27. Its guts have been redesigned to make room for expansion; testers and bug reports are particularly welcome.

Russ Allbery released [podlators 1.11](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00839.html), with several bug-fixes and some new features. (Russ still has a pile of mail to dig through, and he hopes to release another version soon.)
...
Gisle Aas made a brief appearance to post [an ExtUtils::MakeMaker patch](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00805.html) which causes `$ENV{PERL_MM_USE_DEFAULT}`, when set, to instruct `ExtUtils::MakeMaker::prompt` to return the default answer to every question without waiting for input.
...
Charles Lane fixed some niggling problems under VMS, and Craig A. Berry found some new ones. VMSperl now builds with PerlIO as the default, and it is possible to [configure it for 64-bitness](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00601.html) (although this causes a colourful test failure, evidently due to bugs in the system's `gcvt()` function, used to stringify numbers).

Mikhail Zabaluev patched `perldoc` to use `File::Temp` instead of creating temporary files in an ad-hoc fashion. Tim Jenness tried to convince him that he should fix [various other things](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00959.html) in `perldoc`, but Mikhail didn't fall for it. Does anyone else want to try? (One of the changes, to use `Pod::Man` rather than the `pod2man` program, is similar to Schwern's [recent change to installhtml](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00823.html).)

Perlbug posted an intriguing, but somewhat hard-to-decipher [overview of the bug database](https://www.nntp.perl.org/group/perl.perl5.porters/2001/-10/msg00315.html), before dying again. Several "Hello, is this thing on?" messages and a flood of pent-up bug reports later, it is reported to be working again.
