...
Finally, the last step is to wrap it all up as a proper module. You can see the finished product prettied up with PPI's syntax highlighter at [CPAN::Squish](http://ali.as/CPAN/Squish.html). I've added a few additional small features to the basic code described above, but you get the idea. See also [Perl::Squish]({{<mcpan "Perl::Squish" >}}) for more details.
