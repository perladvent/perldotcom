...
-   [Introducing AxKit](/pub/2002/03/12/axkit.html)
...
In this case, we'll be using a relatively recent addition to AxKit's standard toolkit, SAX Machines, integrated in to AxKit thanks to Kip Hampton. (disclaimer: [XML::SAX::Machines](https://metacpan.org/pod/XML::SAX::Machines) is a module I wrote.) The SAX machine we'll create will be a straight pipeline with a few filters, a lot like the pipelines that AxKit uses. This pipeline will dissect directory listings and generate a list of images segmented into rows for easy display purposes. We don't get in to the details of SAX or SAX machines except to bolt together three building blocks; all of the gory details are handled for us by other modules. If you are interested in the gory details, then see [Part One](https://www.xml.com/pub/a/2002/02/13/sax-machines.html) and [Part Two](https://www.xml.com/pub/a/2002/03/20/machines.html) of Kip's article "Introducing XML::SAX::Machines" on [XML.com](https://www.xml.com/).
...
On the other hand, it doesn't make sense to XMLify raw image data (though things like [SVG](http://www.w3.org/TR/SVG/)--covered in [XML.com's Sacre SVG articles](http://www.xml.com/pub/at/22)--and [dia](https://wiki.gnome.org/Apps/Dia/) files are a natural fit), so we'll take advantage of AxKit's integration with Apache and mod\_perl to delegate image handlng to these more suitable tools.
...
My::Thumbnailer uses the relatively new [Imager](https://metacpan.org/pod/Imager) module by Arnar M. Hrafnkelsson and Tony Cook. This is a best-of-breed module that competes with the likes of the venerable [GD](https://metacpan.org/pod/GD), the juggernaut [Image::Magick](https://metacpan.org/pod/Image::Magick), and [Graphics::Libplot](https://metacpan.org/pod/Graphics::Libplot)). Imager is gaining a reputation for speed, quality and a full-featured API.
...
-   [My::ProofSheetMachine](/pub/2002/09/24/axkit.html?page=2#My::ProofSheetMachine) is a short module that builds a [SAX Machine Pipeline](https://metacpan.org/pod/XML::SAX::Machines). SAX filters are used in this application to handle tasks that are more suited to Perl than to XSLT or XSP:
    -   [My::FileList2Data](/pub/2002/09/24/axkit.html?page=2#My::Filelist2Data) is another short module that uses the [XML::Simple](https://metacpan.org/pod/XML::Simple) module from CPAN to convert the `<filelist>` in to a Perl data structure that is passed on. This is its own filter because we want to customize XML::Simple and the resulting data structure a bit before passing it on.
...
-   [rowsplitter.xsl](/pub/2002/09/24/axkit.html?page=3#rowsplitter.xsl) takes each row of images and makes it into two table rows: one for the images and one for the captions. This is easier to do in XSLT than in SAX, so here is where we shift from SAX processing to XSLT processing.
...
When loading a SAX machine into the pipeline, you can give it a simple list of SAX filters ([there are many available on CPAN](https://metacpan.org/search?q=XML%3A%3AFilter)) and it will build a pipeline of them. This is done with a (not shown) `PerlSetVar AxSAXMachineFilters "..."` directive. The limitation with this directive is that you cannot pass in any initialization values to the filters and we want to.
...
[XML::Filter::TableWrapper](https://metacpan.org/pod/XML::Filter::TableWrapper) is a CPAN module is used to take the `<images>` list and segmenting it by insert `<tr>...</tr>` tags around every (it's configurable) `<image>` elements. This configuration is done by the [My::ProofSheetMachine](/pub/2002/09/24/axkit.html?page=2#My::ProofSheetMachine) module we showed earlier:
...
Now, I know several ways to do this in Perl in the AxKit environment and none are so easy for me as using XSLT. [YMMV](https://dictionary.cambridge.org/us/dictionary/english/ymmv).
...
Another approach would be to go back and tweak [My::ProofSheet](/pub/2002/09/24/axkit.html?page=3#My::ProofSheet) to inherit from [XML::Filter::Merger](https://metacpan.org/pod/XML::SAX::Machines) and insert it using a SAX parser. That would be a bit slower, I suspect, because SAX parsing in general tends to be slower than XSLT's internal parsing. It would rob the application of the configurability that having merging as a separate step engenders. By factoring this functionality in to the metamerger.xsl stylesheet, we offer the site designer the ability to pull data from other sources, or even to fly without any metadata at all.
...
The fourth template converts the `<caption>` elements to `<td>` elements and copies all their content through, since [captionstyler.xsl](/pub/2002/09/24/axkit.html?page=4#captionstyler.xsl) already did the presentation for them.
