...
Because old weather is old news, I throw away any values older than three hours, using `DateTime` and [`DateTime::Duration`]({{<mcpan "DateTime::Duration" >}}) methods in a `grep`:
...
This is so, so much easier than fiddling around with epochs and `3*3600` all over the place. If you find yourself writing 3600 anywhere in your code, you should be using `DateTime::Duration` instead. Next, I feed the data points into the [`Chart::Lines`]({{<mcpan "Chart::Lines" >}}) module, a part of the [`Chart`]({{<mcpan "Chart" >}}) distribution. I use this in three phases. First, I create a new `Chart` and specify how large the resulting graphic should be:
