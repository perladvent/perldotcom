...
For an example of how good IDEs have inspired at least one Perl developer, see [Adam Kennedy](/authors/adam-kennedy)'s [Perl.com article on his new PPI module](/pub/2005/06/09/ppi.html) and [Scott Sotka's Devel::Refactor module](https://metacpan.org/pod/Devel::Refactor/) (used in EPIC).
...
Anyone who has installed Perl modules from CPAN has seen unit tests--these are the various, often copious, tests that run when you execute the `make test` part of the installation process. The vast majority of CPAN modules include a suite of tests, often using the [Test::Harness](https://metacpan.org/pod/Test::Harness) and/or [Test::More](https://metacpan.org/pod/Test::More) modules. A good IDE will make it very easy to both create and run unit tests as you develop your project.
...
A more advanced version of this feature might involve having the IDE create stub code to test all of the subroutines in an existing file, or to run all of the scripts in a specified directory under `Test::Harness`, or to run a set of tests using [Test::Unit::TestRunner](https://metacpan.org/pod/Test::Unit::TestRunner) or [Test::Unit::TkTestRunner](https://metacpan.org/pod/Test::Unit::TkTestRunner). (The latter provides a GUI testing framework.)
