...
CPAN has a fair collection of wiki implementations, including Kwiki, [`CGI::Wiki`](https://metacpan.org/pod/CGI::Wiki), [`Apache::MiniWiki`](https://metacpan.org/pod/Apache::MiniWiki), and others. However, there may well be times when it makes sense to create a new implementation, particularly when the wiki is to be embedded in another application.
