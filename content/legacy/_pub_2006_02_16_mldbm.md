...
The secret is Perl's `tie` function. It associates a particular hash (here named `%data_for_ad_on `) with a "class" and a file. A class that works for "complex" data (data containing references) is the [MLDBM](https://metacpan.org/pod/MLDBM) module, available from CPAN.
...
The script shown above uses the [Fcntl](https://metacpan.org/pod/Fcntl) module to make it easier to create or update the DBM database file. The imported ` O_CREAT|O_RDWR` parameters tell the script to create the database file if it doesn't yet exist or to update (read/write) the file if it exists.
