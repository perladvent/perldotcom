...
The `CLOSE` method closes the handle. I could have inherited from [Tie::StdHandle](https://metacpan.org/pod/Tie::StdHandle) to gain this method and many more like it.
...
Note too that [Memoize](https://metacpan.org/pod/Memoize) can be used in non-object situations - in fact the documentation for it doesn't seem to contemplate using it for object factories.
...
There are many other ways to achieve this same effect. For instance, if you think your callers might pass you unexpected arguments, then `Memoize` would make multiple instances, one for each set of arguments. In this case, managing the singleton with modules like [Cache::FastMemoryCache](https://metacpan.org/pod/Cache::FastMemoryCache) from CPAN may make more sense. You could even use a file lexical, assigning it a value in a `BEGIN` block. Remember bless doesn't have to be used in a method. You could say:
