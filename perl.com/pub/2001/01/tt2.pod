...
run the risk of being shunned by polite Perl society.
...
TT2 was designed and written by Andy Wardley <abw@cre.canon.co.uk>. It was 
...
at least, the most I<used>) Perl templating system.
...
TT2 is just as useful in non-web applications.
...
in your code. The constructor function, C<new>, takes a number of optional
...
To process the template, you would call the C<process> method like this

    $tt->process('my_template', \%data)
      || die $tt->error;

We pass two parameters to C<process>, the first is the name of the file
...
So what kinds of things can go in C<%data>? The answer is just about
...
called C<name>, C<season> and C<teams>. Notice that C<teams> is a complex
data structure.
...
The keys of the C<%data> have become the names of the data variables within
the template.

=item *

Template processing directives are surrounded by C<[%> and C<%]> 
sequences.

=item *

If these tags are replaced with C<[%-> C<-%]> then the preceding or 
following linefeed is suppressed.

=item *

In the C<FOREACH> loop, each element of the C<teams> list was assigned,
in turn, to the temporary variable C<team>.

=item *

Each item assigned to the C<team> variable is a Perl hash. Individual
...
to know the data items which will be passed into the template.
...
each team might be an object, in which case C<name>, C<played>, etc. would
...
it needed to call methods rather than access hash values.
...
Therefore if you have a data source which lists the team name togther with
...
Notice that we've introduced the third parameter to C<process>. If this
parameter is missing then the TT2 sends its output to C<STDOUT>. If this
...
or a reference to an object which is assumed to implement a C<print>
method.
...
with a C<foreach> loop and a couple of C<print> statements in your code.
...
Notice that as we've passed the whole object into C<process> then we
need to put an extra level of indirection on our template variables
- everything is now a component of the C<league> variable. Other 
than that, everything in the template is very similar to what we've
used before. Presumably now C<team.name> calls an accessor function 
...
This will process a template called C<league_xml_split.tt> and put the 
results in a file called C<league_split.xml>.
...
Dave Cross <dave@dave.org.uk> is the owner of Magnum Solutions Ltd, 
a London-based Perl consultancy. He was also the founder of 
london.pm. His book I<Data Munging with Perl> has just been published
by Manning.
